<template>
  <div>
    <b-alert class="m-3" :show="!loading && !showResults">No products added yet &middot; <a
        href="/admin/product/create">Create product</a></b-alert>
    ...
    <div class="table-responsive-sm">
      <b-table
        ...
        <template v-slot:cell(image)="data">
          <a href="#" :style="{'background-image': 'url(' + data.item.variants[0].image + ')'}"
             class="variant-image-placeholder dark-link"></a>
        </template>
        <template v-slot:cell(name)="data">
          <span>{{ data.item.name }}</span>
          <b-badge class="badge-soft-danger ml-2" v-if="!data.item.active"><i class="fas fa-check-circle"></i>InActive
          </b-badge>
        </template>
        ...
        <template v-slot:cell(category_price)="data">
          <span>{{ helpers.priceFormat(data.item.variants[0].pricing.price) }}</span>
        </template>
        <template v-slot:cell(actions)="data">
          <a v-b-tooltip:hover title="Delete" class="float-right mr-2" style="width: 10%; cursor: pointer"
             @click="doDelete(data.item)"><i class="ri-delete-bin-6-fill"></i></a>
          ...
          <a v-b-tooltip:hover title="Edit" :href="'/admin/product/edit/' + data.item.id" class="float-right mr-4"
             style="width: 10%"><i class="ri-pencil-fill"></i></a>
        </template>
      </b-table>
    </div>
